...
import (
	...
	"m/test/e2e/framework/consts"
	"m/test/e2e/mock/server"
	"m/test/e2e/mock/server/httpserver"
	"m/test/e2e/mock/server/streamserver"
	"m/test/e2e/pkg/port"
)
...
func NewMockServers(portAllocator *port.Allocator) *MockServers {
	...
	udsAddr := fmt.Sprintf("%s/tool_echo_server_%d.sock", os.TempDir(), udsIndex)
	os.Remove(udsAddr)
