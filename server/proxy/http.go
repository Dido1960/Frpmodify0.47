...
func (pxy *HTTPProxy) GetRealConn(remoteAddr string) (workConn net.Conn, err error) {
	...
	if pxy.cfg.UseEncryption {
		rwc, err = toolIo.WithEncryption(rwc, []byte(pxy.serverCfg.Token))
		if err != nil {
	...
	if pxy.GetLimiter() != nil {
		rwc = toolIo.WrapReadWriteCloser(limit.NewReader(rwc, pxy.GetLimiter()), limit.NewWriter(rwc, pxy.GetLimiter()), func() error {
			return rwc.Close()
		})
	}

	workConn = toolNet.WrapReadWriteCloserToConn(rwc, tmpConn)
	workConn = toolNet.WrapStatsConn(workConn, pxy.updateStatsAfterClosedConn)
