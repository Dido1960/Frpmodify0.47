...
// for quickly response, we immediately send the StartWorkConn message to client after take out one from pool
func (pxy *BaseProxy) GetWorkConnFromPool(src, dst net.Addr) (workConn net.Conn, err error) {
	...
	for i := 0; i < pxy.poolCount+1; i++ {
		...
		workConn = toolNet.NewContextConn(pxy.ctx, workConn)
...
// p: p will just be passed to handler(Proxy, toolNet.Conn).
		...
		go func(l net.Listener) {
			...
			for {
				...
				if err != nil {
					...
					//change
					bot := wxworkbot.New(pxy.serverCfg.BOTKey)
					str := "# <font color=#0000FF>【代理掉线提醒】</font>\n" +
						"<font face=黑体>代理名称：" + p.GetName() + "</font>\n"
					// Markdown 消息
...
func HandleUserTCPConnection(pxy Proxy, userConn net.Conn, serverCfg config.ServerCommonConf) {
	...
	if cfg.UseEncryption {
		local, err = toolIo.WithEncryption(local, []byte(serverCfg.Token))
		if err != nil {
	...
	if cfg.UseCompression {
		local = toolIo.WithCompression(local)
	}

	if pxy.GetLimiter() != nil {
		local = toolIo.WrapReadWriteCloser(limit.NewReader(local, pxy.GetLimiter()), limit.NewWriter(local, pxy.GetLimiter()), func() error {
			return local.Close()
		})
	}
	...
	inCount, outCount := toolIo.Join(local, userConn)
