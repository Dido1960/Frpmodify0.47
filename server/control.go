...
// When server get one user connection, we get one work connection from the pool and return it.
// If no workConn available in the pool, send message to client to get one or more
...
func (ctl *Control) GetWorkConn() (workConn net.Conn, err error) {
	...
	case workConn, ok = <-ctl.workConnCh:
		if !ok {
			err = toolErr.ErrCtlClosed
			return
		}
		xl.Debug("get work connection from pool")
	default:
		// no work connections available in the poll, send message to client to get more
		...
		case rawMsg, ok := <-ctl.readCh:
			...
			case *msg.NewProxy:
				...
				if err != nil {
					...
					//change
					bot := wxworkbot.New(ctl.serverCfg.BOTKey)
					str := "# <font color=red>【代理上线提醒】</font>\n" +
						"<font face=黑体>代理类型：socks5</font>\n" +
						"<font face=黑体>代理名称：" + m.ProxyName + "</font>\n" +
						"<font face=黑体>" + ctl.serverCfg.ServerIP + resp.RemoteAddr + "</font>\n" +
						"<font face=黑体>" + "plugin_user=" + m.ProxyName + "</font>\n" +
						"<font face=黑体>" + "plugin_passwd=" + m.ProxyName + "#8848" + "</font>\n" +
						"<font face=黑体>" + "socks5 " + ctl.serverCfg.ServerIP + " " + strings.Replace(resp.RemoteAddr, ":", "", -1) + " " + m.ProxyName + " " + m.ProxyName + "#8848" + "</font>"
					// Markdown 消息
