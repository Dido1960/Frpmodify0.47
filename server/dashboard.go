...
func (svr *Service) RunDashboardServer(address string) (err error) {
	...
	subRouter.Use(toolNet.NewHTTPAuthMiddleware(user, passwd).Middleware)
	...
	subRouter.PathPrefix("/static/").Handler(toolNet.MakeHTTPGzipHandler(http.StripPrefix("/static/", http.FileServer(assets.FileSystem)))).Methods("GET")
