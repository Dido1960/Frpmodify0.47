...
type Manager struct {
	visitorListeners map[string]*toolNet.CustomListener
...
func NewManager() *Manager {
	return &Manager{
		visitorListeners: make(map[string]*toolNet.CustomListener),
		skMap:            make(map[string]string),
	}
}

func (vm *Manager) Listen(name string, sk string) (l *toolNet.CustomListener, err error) {
	vm.mu.Lock()
	defer vm.mu.Unlock()
...
func (vm *Manager) NewConn(name string, conn net.Conn, timestamp int64, signKey string,
	...
	if l, ok := vm.visitorListeners[name]; ok {
		...
		if useEncryption {
			if rwc, err = toolIo.WithEncryption(rwc, []byte(sk)); err != nil {
		...
		err = l.PutConn(toolNet.WrapReadWriteCloserToConn(rwc, conn))
	} else {
