...
func NewHTTPReverseProxy(option HTTPReverseProxyOptions, vhostRouter *Routers) *HTTPReverseProxy {
	...
	proxy := &httputil.ReverseProxy{
		...
		Director: func(req *http.Request) {
			...
			if rc != nil {
				...
				if rc.ChooseEndpointFn != nil {
					...
					toolLog.Trace("choose endpoint name [%s] for http request host [%s] path [%s] httpuser [%s]",
		...
		ErrorHandler: func(rw http.ResponseWriter, req *http.Request, err error) {
			toolLog.Warn("do http proxy request [host: %s] error: %v", req.Host, err)
...
func (rp *HTTPReverseProxy) GetRouteConfig(domain, location, routeByHTTPUser string) *RouteConfig {
	...
	if ok {
		toolLog.Debug("get new HTTP request host [%s] path [%s] httpuser [%s]", domain, location, routeByHTTPUser)
...
func (l *wrapLogger) Write(p []byte) (n int, err error) {
	toolLog.Warn("%s", string(bytes.TrimRight(p, "\n")))
	return len(p), nil
}
