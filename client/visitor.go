...
import (
	...
	toolIo "github.com/fatedier/golib/io"
...
func (sv *STCPVisitor) handleConn(userConn net.Conn) {
	...
	if sv.cfg.UseEncryption {
		remote, err = toolIo.WithEncryption(remote, []byte(sv.cfg.Sk))
		if err != nil {
	...
	if sv.cfg.UseCompression {
		remote = toolIo.WithCompression(remote)
	}

	toolIo.Join(userConn, remote)
}
...
func (sv *XTCPVisitor) handleConn(userConn net.Conn) {
	...
	// wrap kcp connection
	var remote io.ReadWriteCloser
	remote, err = toolNet.NewKCPConnFromUDP(lConn, true, natHoleRespMsg.ClientAddr)
	if err != nil {
	...
	if sv.cfg.UseEncryption {
		muxConnRWCloser, err = toolIo.WithEncryption(muxConnRWCloser, []byte(sv.cfg.Sk))
		if err != nil {
			xl.Error("create encryption stream error: %v", err)
			return
		}
	...
	if sv.cfg.UseCompression {
		muxConnRWCloser = toolIo.WithCompression(muxConnRWCloser)
	}

	toolIo.Join(userConn, muxConnRWCloser)
...
func (sv *SUDPVisitor) dispatcher() {
	...
	for {
		...
		case firstPacket = <-sv.sendCh:
			if firstPacket == nil {
				xl.Info("client sudp visitor proxy is closed")
				return
			}
		...
		if err != nil {
			xl.Warn("newVisitorConn to server error: %v, try to reconnect", err)
			continue
		}
...
func (sv *SUDPVisitor) worker(workConn net.Conn, firstPacket *msg.UDPPacket) {
	...
	// udp service -> client -> server -> client visitor -> user
	workConnReaderFn := func(conn net.Conn) {
		...
		for {
			...
			// client will send heartbeat in workConn to client visitor for keeping alive
			...
			case *msg.Ping:
				xl.Debug("client visitor get ping message from client")
				continue
			case *msg.UDPPacket:
				if errRet := errors.PanicToError(func() {
					sv.readCh <- m
					xl.Trace("client visitor get udp packet from workConn: %s", m.Content)
				}); errRet != nil {
...
func (sv *SUDPVisitor) getNewVisitorConn() (net.Conn, error) {
	...
	visitorConn, err := sv.ctl.connectServer()
	if err != nil {
		return nil, fmt.Errorf("client connect server error: %v", err)
	}
	...
	err = msg.WriteMsg(visitorConn, newVisitorConnMsg)
	if err != nil {
		return nil, fmt.Errorf("client send newVisitorConnMsg to server error: %v", err)
	}
	...
	err = msg.ReadMsgInto(visitorConn, &newVisitorConnRespMsg)
	if err != nil {
		return nil, fmt.Errorf("client read newVisitorConnRespMsg error: %v", err)
	}
	_ = visitorConn.SetReadDeadline(time.Time{})
	...
	return toolNet.WrapReadWriteCloserToConn(remote, visitorConn), nil
}
