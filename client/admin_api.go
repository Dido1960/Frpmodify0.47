...
func (svr *Service) apiReload(w http.ResponseWriter, r *http.Request) {
	...
	//	res.Code = 400
	//	res.Msg = err.Error()
	//	log.Warn("reload client proxy config error: %s", res.Msg)
	//	return
	//}
	//
...
func (svr *Service) apiGetConfig(w http.ResponseWriter, r *http.Request) {
	...
	if svr.cfgFile == "" {
		res.Code = 400
		res.Msg = "client has no config file path"
		log.Warn("%s", res.Msg)
		return
	}
	...
	if err != nil {
		res.Code = 400
		res.Msg = err.Error()
		log.Warn("load client config file error: %s", res.Msg)
		return
	}
...
func (svr *Service) apiPutConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		res.Code = 500
		res.Msg = fmt.Sprintf("write content to client config file error: %v", err)
		log.Warn("%s", res.Msg)
		return
	}
}
