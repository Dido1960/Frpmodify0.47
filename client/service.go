...
type Service struct {
	// uniq id got from server, attach it in loginMsg
	runID string
	...
	// This is configured by the login response from server
	serverUDPPort int
...
func (svr *Service) Run() error {
	...
	if svr.cfg.DNSServer != "" {
		...
		// Change default dns server for client
...
func (svr *Service) keepControllerWorking() {
	...
	// if client reconnect server, we need to limit retry times in 1min
...
// login creates a connection to server and registers it self as a client
...
func (cm *ConnectionManager) OpenConnection() error {
	...
	if strings.EqualFold(cm.cfg.Protocol, "quic") {
		...
		tlsConfig.NextProtos = []string{"tool"}
...
func (cm *ConnectionManager) Connect() (net.Conn, error) {
	if cm.quicConn != nil {
		...
		return toolNet.QuicStreamToNetConn(stream, cm.quicConn), nil
...
func (cm *ConnectionManager) realConnect() (net.Conn, error) {
	...
	if protocol == "websocket" {
		protocol = "tcp"
		dialOptions = append(dialOptions, libdial.WithAfterHook(libdial.AfterHook{Hook: toolNet.DialHookWebsocket()}))
	}
	...
	dialOptions = append(dialOptions,
		...
		libdial.WithAfterHook(libdial.AfterHook{
			Hook: toolNet.DialHookCustomTLSHeadByte(tlsConfig != nil, cm.cfg.DisableCustomTLSFirstByte),
		}),
	)
