...
func (pxy *XTCPProxy) InWorkConn(conn net.Conn, m *msg.StartWorkConn) {
	...
	kcpConn, err := toolNet.NewKCPConnFromUDP(lConn, false, uAddr.String())
	if err != nil {
...
func (pxy *UDPProxy) InWorkConn(conn net.Conn, m *msg.StartWorkConn) {
	...
	var err error
	if pxy.limiter != nil {
		rwc = toolIo.WrapReadWriteCloser(limit.NewReader(conn, pxy.limiter), limit.NewWriter(conn, pxy.limiter), func() error {
			return conn.Close()
		})
	}
	if pxy.cfg.UseEncryption {
		rwc, err = toolIo.WithEncryption(rwc, []byte(pxy.clientCfg.Token))
		if err != nil {
			conn.Close()
	...
	if pxy.cfg.UseCompression {
		rwc = toolIo.WithCompression(rwc)
	}
	conn = toolNet.WrapReadWriteCloserToConn(rwc, conn)
...
func (pxy *SUDPProxy) InWorkConn(conn net.Conn, m *msg.StartWorkConn) {
	...
	workConnReaderFn := func(conn net.Conn, readCh chan *msg.UDPPacket) {
		...
		for {
			...
			select {
			case <-pxy.closeCh:
				xl.Trace("client sudp proxy is closed")
				return
			default:
			}
	...
	workConnSenderFn := func(conn net.Conn, sendCh chan msg.Message) {
		...
		for rawMsg := range sendCh {
			switch m := rawMsg.(type) {
			case *msg.UDPPacket:
				xl.Trace("client send udp package to client visitor, [udp local: %v, remote: %v], [tcp work conn local: %v, remote: %v]",
					m.LocalAddr.String(), m.RemoteAddr.String(), conn.LocalAddr().String(), conn.RemoteAddr().String())
			case *msg.Ping:
				xl.Trace("client send ping message to client visitor")
			}
...
func HandleTCPWorkConnection(ctx context.Context, localInfo *config.LocalSvrConf, proxyPlugin plugin.Plugin,
	...
	remote = workConn
	if limiter != nil {
		remote = toolIo.WrapReadWriteCloser(limit.NewReader(workConn, limiter), limit.NewWriter(workConn, limiter), func() error {
	...
	if baseInfo.UseEncryption {
		remote, err = toolIo.WithEncryption(remote, encKey)
		if err != nil {
			workConn.Close()
			xl.Error("create encryption stream error: %v", err)
			return
